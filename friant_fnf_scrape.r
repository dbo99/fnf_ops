...
c_mil_fnf <- read.table(text = substring(c_rawtext, regexpr(start, c_rawtext), regexpr(end, c_rawtext)))
rm(start, end, c_rawtext)
}

{
# rename current report columns #
c_mil_fnf  <- c_mil_fnf %>% rename("monthday" = !!names(.[1]),
                                                           "Edison" = !!names(.[2]),
                                                           "Florence" = !!names(.[3]),
                                                           "Huntington" = !!names(.[4]),
                                                           "Shaver" = !!names(.[5]),
...
c_mil_fnf <- c_mil_fnf %>% transmute(monthday, mil_fnf)
  
  }
head(c_mil_fnf)

c_mil_fnf <- c_mil_fnf %>% mutate(date = paste0(c_mil_mon,"/", monthday,"/", c_mil_yr)) %>%
                           transmute(chps_date = mdy(date) + 1, mil_fnf)
head(c_mil_fnf)

### scrape previous report ###
start <- "\n    1"
end <- "\n  TOTALS"
p_mil_fnf <- read.table(text = substring(p_rawtext, regexpr(start, p_rawtext), regexpr(end, p_rawtext)))
...
p_mil_fnf  <- p_mil_fnf %>% rename("monthday" = !!names(.[1]),
                                     "Edison" = !!names(.[2]),
                                     "Florence" = !!names(.[3]),
                                     "Huntington" = !!names(.[4]),
                                     "Shaver" = !!names(.[5]),
                                     ...
                                     "mil_fnf" = !!names(.[14]),
                                     "Millerton_natriver_wyaccum" = !!names(.[15])) 
  p_mil_fnf <- p_mil_fnf %>% transmute(monthday, mil_fnf)
  
}
head(p_mil_fnf)

p_mil_fnf <- p_mil_fnf %>% mutate(date = paste0(p_mil_mon,"/", monthday,"/", p_mil_yr)) %>%
  transmute(chps_date = mdy(date) + 1, mil_fnf)
head(p_mil_fnf)

mil_fnf <- rbind(c_mil_fnf, p_mil_fnf) %>% arrange(desc(chps_date))
mil_fnf <- head(mil_fnf,12)
mil_fnf
